...
_format_ <data_device> <hash_device>
...
_open_ <data_device> <name> <hash_device> <root_hash> +
_open_ <data_device> <name> <hash_device> --root-hash-file <path> +
_create_ <name> <data_device> <hash_device> <root_hash> (*OBSOLETE
syntax*)
...
_verify_ <data_device> <hash_device> <root_hash> +
_verify_ <data_device> <hash_device> --root-hash-file <path>
...
For backward compatibility there is *remove* command alias for *close*
command.
...
Path to roothash signature file used to verify the root hash (in
...
Copyright © 2012-2022 Milan Broz
