...
You can format the device with a LUKS2 header if you specify *--type luks2* in the *luksFormat* command. For activation, the format is already recognized automatically.
...
*Previously used partitions:* If a partition was previously used, it is a very good idea to wipe filesystem signatures, data, etc., before creating a LUKS or plain dm-crypt container. For a quick removal of filesystem signatures, use *wipefs*(8).
This may not remove everything.
In particular, MD RAID signatures at the end of a device may survive.
It also does not remove data. For a full wipe, overwrite the whole partition before creating a container.
