...
For a LUKS2 device with hardware OPAL2 (Self Encrypting Drive) support, the --dump-hw-opal option additionally dumps the associated OPAL2 locking range information.
...
Dump the OPAL information (subsystem status and configured locking ranges) of a device that provides an *OPAL* interface.
The device does not need to contain LUKS2 metadata; an OPAL Admin password is requested to query the drive.
See *cryptsetup-opaldump*(8).

To dump the OPAL locking range associated with an existing LUKS2 device instead, use *luksDump* with the --dump-hw-opal option.
