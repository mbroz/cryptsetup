...
cryptsetup-refresh - refreshe parameters of an active mapping

== SYNOPSIS

*cryptsetup <options> _refresh_ <name>*

== DESCRIPTION

Refreshes parameters of active mapping <name>.

Updates parameters of active device <name> without need to deactivate
...
encryption), plain crypt and loopaes.
...
--disable-keyring parameter refreshes a device with volume key passed in
dm-crypt driver.
