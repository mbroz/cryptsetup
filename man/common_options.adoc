...
ifndef::ACTION_BENCHMARK,ACTION_BITLKDUMP,ACTION_TCRYPTDUMP,ACTION_OPALDUMP[]
*--disable-locks*::
...
*--disable-sum*::
Disable SUM (Single User Mode) OPAL2 extension during luksFormat with HW encryption profile (OPAL2).
The option affects only devices not yet enabled for OPAL2 encryption (in factory settings). For devices
with already activated OPAL2 HW encryption profile the option is ignored. The option must be combined
with --hw-opal or --hw-opal-only options.
endif::[]
...
ifdef::ACTION_LUKSDUMP[]
*--dump-hw-opal*::
For _luksDump_, print additional information about the SED OPAL2 hardware locking range associated with a LUKS2 device formatted with hardware OPAL2 (Self Encrypting Drive) support.
The device must be unlocked to query the drive, so a passphrase, a LUKS2 token or the volume key has to be supplied.
See also the *opaldump* action in *cryptsetup-opaldump*(8) for dumping OPAL information of the whole device (without attachment to LUKS2 metadata).
endif::[]
...
ifndef::ACTION_BENCHMARK,ACTION_BITLKDUMP,ACTION_OPALDUMP[]
...
ifdef::ACTION_OPEN,ACTION_LUKSFORMAT,ACTION_LUKSADDKEY,ACTION_LUKSCHANGEKEY,ACTION_LUKSCONVERTKEY,ACTION_LUKSREMOVEKEY,ACTION_LUKSKILLSLOT,ACTION_LUKSDUMP,ACTION_OPALDUMP,ACTION_REENCRYPT,ACTION_REPAIR,ACTION_LUKSRESUME,ACTION_RESIZE,ACTION_TCRYPTDUMP,ACTION_BITLKDUMP[]
*--timeout*, *-t* _seconds_::
