...
extern char *vk_files[];
extern char *vks_in_keyring[];
extern uint32_t key_sizes[];
extern int vk_files_count;
extern int key_sizes_count;
extern int vks_in_keyring_count;
...
static void _set_reencryption_flags(uint32_t *flags)
{
	...
	if (ARG_SET(OPT_VOLUME_KEY_FILE_ID) && ARG_SET(OPT_NEW_VOLUME_KEY_FILE_ID))
		*flags |= CRYPT_REENCRYPT_DIRECT_NEW_KEY;
}
...
{
	int r, tries, new_keysize_bytes = 0;
	const char *vk_file1 = NULL, *vk_file2 = NULL;
	...
	/* Either --volume-key-file or --volume-key-keyring option were used */
	if (ARG_SET(OPT_VOLUME_KEY_FILE_ID) || ARG_SET(OPT_VOLUME_KEY_KEYRING_ID)) {
		vk_file1 = vk_files[0];
		vk_file2 = vk_files[1];
		new_keysize_bytes = key_sizes[1] / 8;

		/*
		 * Allows reuse of --new-volume-key-file and --new-key-size for second (new) volume key
		 * from reencryption initialization.
		 */
		if (!vk_file2 && ARG_SET(OPT_NEW_VOLUME_KEY_FILE_ID))
			vk_file2 = ARG_STR(OPT_NEW_VOLUME_KEY_FILE_ID);
		if (!new_keysize_bytes && ARG_SET(OPT_NEW_KEY_SIZE_ID))
			new_keysize_bytes = ARG_UINT32(OPT_NEW_KEY_SIZE_ID) / 8;

		r = init_keyslot_contexts_non_interactive(cd, vk_file1, vk_file2, key_sizes[0] / 8, new_keysize_bytes, vks_in_keyring[0], vks_in_keyring[1], &kc1, &kc2);
		if (r < 0)
			return r;

		r = crypt_activate_by_keyslot_context(cd, NULL, ARG_INT32(OPT_KEY_SLOT_ID),
						      kc1, ARG_INT32(OPT_NEW_KEY_SLOT_ID), kc2 ?: kc1,
						      0);
	} else {
		...
		do {
			...
			kc1 = NULL;
			r = init_keyslot_context(cd, NULL, verify_passphrase(0), false, &kc1);
			if (r < 0)
				goto out;

			r = crypt_activate_by_keyslot_context(cd, NULL, ARG_INT32(OPT_KEY_SLOT_ID),
							      kc1, ARG_INT32(OPT_NEW_KEY_SLOT_ID), kc1,
							      0);
...
{
	const char *vk_description = "blabla";
	char *key = NULL, *vk_description_activation1 = NULL;
	...
	if (ARG_SET(OPT_VOLUME_KEY_FILE_ID)) {
		...
		if (!keysize && !ARG_SET(OPT_KEY_SIZE_ID)) {
			log_err(_("Cannot determine volume key size for LUKS, please use --key-size option."));
			r = -EINVAL;
			goto out;
		} else if (!keysize)
			keysize = ARG_UINT32(OPT_KEY_SIZE_ID) / 8;
		...
		r = -ENOTSUP;
			r = tools_parse_vk_description(vk_description, &vk_description_activation1);
			if (r < 0)
				goto out;
			r = crypt_keyslot_context_init_by_vk_in_keyring(cd, vk_description_activation1, &kc);
			if (r)
				goto out;
	} else {
		r = try_token_unlock(cd, ARG_INT32(OPT_KEY_SLOT_ID),
		...
		do {
			...
			kc = NULL;
			r = init_keyslot_context(cd, NULL, verify_passphrase(0), false, &kc);
			if (r < 0)
				goto out;
...
static int reencrypt_luks2_load(struct crypt_device *cd, const char *data_device)
{
	...
	if (r >= 0)
		r = crypt_reencrypt_init_by_keyslot_context(cd, active_name, kc, kc2 ?: kc, ARG_INT32(OPT_KEY_SLOT_ID),
				ARG_INT32(OPT_NEW_KEY_SLOT_ID), NULL, NULL, &params);
out:
	free(hash);
	crypt_keyslot_context_free(kc);
	crypt_keyslot_context_free(kc2);
...
static struct crypt_keyslot_context *try_token(struct crypt_device *cd, struct keyslot_contexts *kcs, int keyslot)
{
	unsigned i;
	...
	for (i = 0; i < kcs->last_tkn; i++) {
		...
		if (crypt_token_is_assigned(cd, kcs->tkns[i].id, keyslot) == 0 &&
		    crypt_activate_by_keyslot_context(cd, NULL, keyslot, pkc, CRYPT_ANY_SLOT, NULL, 0) == keyslot) {
				log_dbg("Keyslot %d successfully unlocked using token id %d.", keyslot, kcs->tkns[i].id);
...
static struct crypt_keyslot_context *try_unlock(struct crypt_device *cd, struct keyslot_contexts *kcs, int keyslot)
{
	unsigned i;
	...
	log_dbg("Retrying cached keyslot contexts to unlock keyslot %d.", keyslot);

	for (i = 0; i < kcs->last_kc; i++) {
		if (crypt_activate_by_keyslot_context(cd, NULL, keyslot, kcs->kc[i], CRYPT_ANY_SLOT, NULL, 0) == keyslot) {
			log_dbg("Keyslot %d successfully unlocked using cached keyslot context.", keyslot);
			tools_keyslot_msg(keyslot, UNLOCKED);
			return kcs->kc[i];
		}
	}
...
static struct crypt_keyslot_context *reencrypt_get_token_context(struct keyslot_contexts *kcs, int token)
{
	unsigned i;
...
static void reencrypt_token_add(struct keyslot_contexts *kcs, int token, struct crypt_keyslot_context *token_kc)
{
	unsigned i;
...
static void reencrypt_keyslot_unlocked_by_context(struct keyslot_contexts *kcs, int keyslot, struct crypt_keyslot_context *kc)
{
	unsigned i;
...
static void reencrypt_token_unlocks_keyslot(struct keyslot_contexts *kcs, int token, int keyslot, struct crypt_keyslot_context *token_kc)
{
	assert(kcs);
	assert(token_kc);
...
static void reencrypt_keyslot_unlocked_by_context_new(struct keyslot_contexts *kcs, int keyslot, struct crypt_keyslot_context *kc)
{
	assert(kcs);
	assert(kc);
	assert(keyslot >= 0);
			...
			struct keyslot_contexts *kcs)
{
	...
	if (kc) {
		r = crypt_activate_by_keyslot_context(cd, NULL, slot_to_check, kc, CRYPT_ANY_SLOT, NULL, 0);
		if (r == slot_to_check) {
	...
	r = try_token_unlock(cd, slot_to_check, token, NULL,
			   ...
			   struct crypt_device *cd, const char *msg, int slot_to_check)
{
	...
	if (kc) {
		log_dbg("Keyslot %d can be unlocked using token.", slot_to_check);
		reencrypt_keyslot_unlocked_by_context(kcs, slot_to_check, kc);
	...
	/* FIXME: drop this? */
	/* try already initialized passphrase based keyslot context for another one */
	kc = try_unlock(cd, kcs, slot_to_check);
	if (kc) {
		reencrypt_keyslot_unlocked_by_context(kcs, slot_to_check, kc);
	...
	do {
		r = init_keyslot_context(cd, msg, verify_passphrase(0), false, &kc);
		if (r < 0)
			return r;
...
static int count_token_assigned_to_keyslot(struct crypt_device *cd,
		struct keyslot_contexts *kcs,
		int token,
		int keyslot)
{
	...
	log_dbg("Keyslot %d candidate for reencryption.", keyslot);

	reencrypt_keyslot_add_for_unlock(kcs, keyslot);
	return 1;
}

static int count_single_token_assigned_keyslots(struct crypt_device *cd,
		struct keyslot_contexts *kcs,
		int token,
		int keyslot,
		const char *token_type)
{
	...
	if (keyslot != CRYPT_ANY_SLOT)
		return count_token_assigned_to_keyslot(cd, kcs, token, keyslot);

	for (ks = 0; ks < crypt_keyslot_max(CRYPT_LUKS2); ks++) {
		r = count_token_assigned_to_keyslot(cd, kcs, token, ks);
		if (r < 0)
			return r;
 ...
 * parameters
 */
static int count_token_assigned_keyslots(struct crypt_device *cd, struct keyslot_contexts *kcs, int token, const char *token_type, int keyslot)
{
	int r;
	unsigned count = 0;

	if (token != CRYPT_ANY_TOKEN)
		return count_single_token_assigned_keyslots(cd, kcs, token, keyslot, token_type);

	for (token = 0; token < crypt_token_max(CRYPT_LUKS2); token++) {
		r = count_single_token_assigned_keyslots(cd, kcs, token, keyslot, token_type);
		if (r < 0)
			return r;
...
static int reencrypt_init_keyslot_contexts(struct crypt_device *cd,
					   struct keyslot_contexts *kcs,
					   bool vk_change,
					   bool only_token_keyslots)
{
	int i, new_vk_slot = (vk_change ? 1 : 0), max = crypt_keyslot_max(CRYPT_LUKS2), unlocked, active = 0, unbound = 0;

	i = count_token_assigned_keyslots(cd, kcs, ARG_INT32(OPT_TOKEN_ID_ID),
	...
	for (i = 0; i < max; i++) {
		...
		case CRYPT_SLOT_ACTIVE_LAST:
			...
			active++;
			if (!only_token_keyslots && !reencrypt_keyslot_for_unlock(kcs, i) && (!ARG_SET(OPT_KEY_SLOT_ID) || ARG_INT32(OPT_KEY_SLOT_ID) == i)) {
				reencrypt_keyslot_add_for_unlock(kcs, i);
				unlocked++;
				log_dbg("Keyslot %d may be used for reencryption by passphrase prompt.", i);
			}
			break;
		case CRYPT_SLOT_UNBOUND:
			unbound++;
...
static int reencrypt_unlock_keyslots(struct crypt_device *cd,
		struct keyslot_contexts *kcs, bool vk_change)
{
	...
	/* FIXME: --key-file only when value is not stdin ("-") */
				      ...
				      struct keyslot_contexts *kcs)
{
	...
	for (i = 0; i < kcs->last_ks; i++) {
		...
		log_dbg("single_token returned %d.", r);
...
static int reencrypt_initialize_keyslot_contexts(struct crypt_device *cd,
		...
		bool prefer_token,
		struct keyslot_contexts *kcs,
		struct crypt_keyslot_context **r_kc_new)
{
	int r;
	struct crypt_keyslot_context *kc = NULL, *kc_new = NULL;
	...
	if (vk_change) {
		r = crypt_keyslot_context_init_by_volume_key(cd, vk_new, vk_new_size, &kc_new);
		if (r < 0)
			return r;
	}

	if (vk) {
		r = crypt_keyslot_context_init_by_volume_key(cd, vk, vk_size, &kc);
		if (r < 0)
			goto err;

		kcs->kc[kcs->last_kc++] = kc;
		kcs->ks[kcs->last_ks].id = kcs->ks[kcs->last_ks].new_id = CRYPT_ANY_SLOT;
		kcs->ks[kcs->last_ks++].p_kc = kc;
	...
	r = reencrypt_init_keyslot_contexts(cd, kcs, vk_change, prefer_token);
	if (r)
		goto err;

	/* Returns total number of token unlocked keyslots or negative errno otherwise */
	...
	if (!prefer_token) {
		/* Now unlock remaining keyslots prompting for passphrase */
		r = reencrypt_unlock_keyslots(cd, kcs, vk_change);
		if (r < 0)
			goto err;
	}
...
static int reencrypt_luks2_init(struct crypt_device *cd, const char *data_device)
{
	...
	char cipher[MAX_CIPHER_LEN], mode[MAX_CIPHER_LEN], *vk = NULL, *vk_new = NULL, *active_name = NULL;
	uint32_t new_key_flags = CRYPT_VOLUME_KEY_NO_SEGMENT;
	const char *new_cipher = NULL;
	struct crypt_keyslot_context *new_key_kc = NULL;
	struct keyslot_contexts kcs = {};
	...
	else if (ARG_SET(OPT_KEY_SIZE_ID))
		/* backward compatible use for --key-size when --new-key-size is unused */
		new_key_size = ARG_UINT32(OPT_KEY_SIZE_ID);

	/* no active keyslot case */
	if (!key_size && ARG_SET(OPT_KEY_SIZE_ID) && ARG_SET(OPT_NEW_KEY_SIZE_ID))
	...
	if (new_key_size || new_cipher)
		...
		new_key_size = crypt_get_volume_key_size(cd);
	...
	if (vk_change && ARG_SET(OPT_NEW_VOLUME_KEY_FILE_ID) && ARG_SET(OPT_VOLUME_KEY_FILE_ID)) {
		if (!key_size) {
			/* No keyslot assigned to default segment */
			log_err(_("Unable to get key size. Provide --key-size option."));
			return -EINVAL;
		}
		...
		if (!crypt_volume_key_verify(cd, vk_new, (size_t)new_key_size)) {
	...
	/* backward compatible option where --volume-key-file was meant to load new key from a file */
	if (vk_change && ARG_SET(OPT_VOLUME_KEY_FILE_ID) && !ARG_SET(OPT_NEW_VOLUME_KEY_FILE_ID)) {
		r = tools_read_vk(ARG_STR(OPT_VOLUME_KEY_FILE_ID), &vk_new, new_key_size);
		if (r < 0)
			goto out;
	...
	r = reencrypt_initialize_keyslot_contexts(cd, vk, vk_new, (size_t)key_size, (size_t)new_key_size, vk_change, prefer_token, &kcs, &new_key_kc);
	crypt_safe_free(vk);
	crypt_safe_free(vk_new);
	vk_new = NULL;
	if (r < 0)
	...
	if (vk_change && !vk) {
		for (i = 0; i < kcs.last_ks; i++) {
			...
			if (!(new_key_flags & CRYPT_VOLUME_KEY_DIGEST_REUSE)) {
				r = crypt_keyslot_add_by_keyslot_context(cd, CRYPT_ANY_SLOT, new_key_kc,
				...
				if (vk_generated) {
					...
					r = crypt_volume_key_get_by_keyslot_context(cd, kcs.ks[i].new_id, vk_new, &(size_t){new_key_size}, kcs.ks[i].p_kc);
					if (r < 0)
						break;

					r = crypt_keyslot_context_init_by_volume_key(cd, vk_new, (size_t)new_key_size, &new_key_kc);
					crypt_safe_free(vk_new);
					vk_new = NULL;
					if (r < 0)
						break;
	...
	r = crypt_reencrypt_init_by_keyslot_context(cd,
			...
			kcs.ks[0].p_kc, new_key_kc ?: kcs.ks[0].p_kc,
			kcs.ks[0].id, kcs.ks[0].new_id,
			cipher, mode, &params);
	...
	if (r < 0 && crypt_reencrypt_status(cd, NULL) == CRYPT_REENCRYPT_NONE) {
		for (i = 0; i < kcs.last_ks; i++) {
			if (kcs.ks[i].new_id >= 0 && kcs.ks[i].new_id != kcs.ks[i].id &&
				...
				log_dbg("Failed to remove keyslot %d with unbound key.", kcs.ks[i].new_id);
		}
	}
