...
int luksFormat(struct crypt_device **r_cd, struct crypt_keyslot_context **r_kc)
{
	...
	r = init_keyslot_context(cd, NULL, verify_passphrase(1),
...
static int action_open_luks(void)
{
	...
	if (ARG_SET(OPT_VOLUME_KEY_FILE_ID) || ARG_SET(OPT_VOLUME_KEY_KEYRING_ID)) {
		r = init_keyslot_contexts_by_volume_keys(cd, vk_files[0], vk_files[1],
							 key_sizes[0] / 8, key_sizes[1] / 8,
							 vks_in_keyring[0], vks_in_keyring[1],
							 &kc1, &kc2);
		if (r < 0)
			goto out;
...
static int action_luksAddKey(void)
{
	...
	char *key = NULL, *vk_description = NULL;
	struct crypt_device *cd = NULL;
	...
	if (ARG_SET(OPT_VOLUME_KEY_FILE_ID)) {
		...
		r = init_keyslot_context(cd, _("Enter any existing passphrase: "),
...
static int luksDump_with_volume_key(struct crypt_device *cd)
{
	...
	r = init_keyslot_context(cd, NULL, false, false, &kc);
	if (r < 0)
		goto out;
...
static const char *verify_reencrypt(void)
{
	...
	if (ARG_SET(OPT_VOLUME_KEY_FILE_ID) && ARG_SET(OPT_KEEP_KEY_ID) && !ARG_SET(OPT_NEW_VOLUME_KEY_FILE_ID))
		return _("Options --volume-key-file (compatible option for new volume key) and --keep-key cannot be combined.");
		 ...
		 void *data __attribute__((unused)))
{
	...
	case OPT_KEY_SIZE_ID:
		...
		if (key_sizes_count < 2)
			...
			if (snprintf(buf, sizeof(buf), _("At most %d key size specifications can be supplied."), 2) < 0)
				buf[0] = '\0';
			usage(popt_context, EXIT_FAILURE,
			      buf,
			      poptGetInvocationName(popt_context));
