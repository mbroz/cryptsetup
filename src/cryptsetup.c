...
static const char *opal_lock_mode_to_string(crypt_hw_opal_lock_state lock)
{
	switch (lock) {
	case CRYPT_HW_OPAL_RW:
		return "read-write";
	case CRYPT_HW_OPAL_RO:
		return "read-only";
	case CRYPT_HW_OPAL_LOCKED:
	default:
		return "locked";
	}
}
...
static void print_opal_table_line(const struct crypt_hw_opal_range *resp)
{
	assert(resp);
	...
	log_std("%-3d %-17" PRIu64 " %-17" PRIu64 " %-11s %-12s %-10s %-8s %-11s\n",
		...
		opal_lock_mode_to_string(resp->lock_state),
...
static int luksDump_with_opal(struct crypt_device *cd)
{
	...
	struct crypt_hw_opal_range response;
	...
	r = crypt_get_hw_opal_locking_ranges_by_keyslot_context(cd, ARG_INT32(OPT_KEY_SLOT_ID),
								kc1, &response, 1);
	if (!r) {
	...
	print_opal_table_line(&response);
...
static int action_opaldump(void)
{
	int r;
	crypt_status_hw_encryption_info ci;
	struct crypt_hw_opal_range response[9];
	char *admin_pin = NULL;
	...
	if (ci == CRYPT_HW_OPAL || ci == CRYPT_HW_OPAL_SUM) {
		r = crypt_get_hw_opal_locking_ranges(cd, admin_pin, admin_pin_size, response, ARRAY_SIZE(response));
		if (!r) {
			log_std(_("No HW OPAL segment configured on device %s.\n"), action_argv[0]);
			r = -ENOENT;
		} else if (r == -EPERM) {
