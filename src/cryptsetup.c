		...
		int keyring_links_count)
{
	...
	if (keyring_links_count > 1) {
		...
		if ((type_part_out1 && type_part_out2) && strcmp(type_part_out1, type_part_out2)) {
			log_err(_("Key types have to be the same for both linked VKs ('%s' not equal to '%s')."),
					type_part_out1, type_part_out2);
			r = -EINVAL;
			goto out;
		}
		if ((keyring_part_out1 && keyring_part_out2) && strcmp(keyring_part_out1, keyring_part_out2)) {
			log_err(_("Keyrings to link have to be the same for both linked VKs ('%s' not equal to '%s')."),
					keyring_part_out1, keyring_part_out2);
			r = -EINVAL;
			goto out;
		}
	}
