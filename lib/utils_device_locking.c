...
static int open_lock_dir(struct crypt_device *cd, const char *dir, const char *base)
{
	...
	if (lockdfd < 0) {
		if (errno == ENOENT) {
			/* Can anyone make the message shorter and more clear, please?:) */
			log_std(cd, _("WARNING: Locking directory %s/%s is missing!\n" \
				      "It's a distro configuration error. Cryptsetup's fallback method\n" \
				      "might result in wrong access rights being set on the directory.\n"), dir, base);
