		...
		struct volume_key **vks)
{
	...
	r = reencrypt_make_backup_segments(cd, hdr, -1, NULL, data_offset, params);
	if (r) {
		...
		struct volume_key **vks)
{
	...
	if (params->mode != CRYPT_REENCRYPT_DECRYPT &&
	    (!params->luks2 || !(cipher && cipher_mode) ||
	     (keyslot_new < 0 && !(params->flags & CRYPT_REENCRYPT_DIRECT_NEW_KEY))))
		return -EINVAL;
	...
	if (params->flags & CRYPT_REENCRYPT_DIRECT_NEW_KEY) {
	...
	const struct crypt_params_reencrypt *params)
{
	...
	if (params && (params->flags & CRYPT_REENCRYPT_DIRECT_NEW_KEY) &&
	    (!kc_new || (kc_new->type != CRYPT_KC_TYPE_KEY && kc_new->type != CRYPT_KC_TYPE_VK_KEYRING) ||
	...
	if (cipher && !crypt_cipher_wrapped_key(cipher, cipher_mode)) {
		/* FIXME: Add support for reading volume key size from CRYPT_KC_TYPE_VK_KEYRING type */
		if (keyslot_new == CRYPT_ANY_SLOT && kc_new && kc_new->type == CRYPT_KC_TYPE_KEY)
			r = kc_new->u.k.volume_key_size;
		else
