			 ...
			 char **json_area, uint64_t offset, int secondary)
{
	 ...
	 */
	if (read_lseek_blockwise(devfd, device_block_size(device), hdr_disk,
	...
	*json_area = malloc(hdr_json_size);
	...
	if (read_lseek_blockwise(devfd, device_block_size(device), *json_area,
			   hdr_json_size, offset + LUKS2_HDR_BIN_LEN) != (ssize_t)hdr_json_size) {
		close(devfd);
		free(*json_area);
		*json_area = NULL;
		return -EIO;
		   ...
		   const char *json_area, int secondary)
{
	 ...
	 */
	if (write_lseek_blockwise(devfd, device_block_size(device), (char *)&hdr_disk,
			...
			struct device *device, int do_recovery)
{
	...
	if (state_hdr1 == HDR_FAIL_IO || state_hdr2 == HDR_FAIL_IO)
		return -EIO;
...
int LUKS2_hdr_version_unlocked(struct crypt_device *cd)
{
	...
	if ((read_lseek_blockwise(devfd, device_block_size(device), &hdr, sizeof(hdr), 0)
