...
struct opal_lr_react {
	struct opal_key key; /* aligned */
	__u32 sum;
	__u8 num_lrs;
	__u8 lr[OPAL_MAX_LRS];
	__u8 set_new_admin_key;
	__u8 range_policy; /* aligned */
...
#define IOC_OPAL_LR_SETUP_START_LENGTH _IOW('p', 243, struct opal_user_lr_setup)
#define IOC_OPAL_LR_ENABLE_DISABLE  _IOW('p', 244, struct opal_user_lr_setup)
...
static int opal_sum_supported(struct crypt_device *cd, struct device *dev)
{
	...
	if (r) {
		/* device advertise SUM feature, now check if kernel supports necessary
