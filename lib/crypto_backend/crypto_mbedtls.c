...
static const mbedtls_md_info_t *crypt_get_hash(const char *name)
{
	...
	for (i = 0; i < sizeof(kHash) / sizeof(*kHash); ++i) {
...
int crypt_backend_init(bool fips)
{
	...
	if (mbedtls_ctr_drbg_seed(&g_ctr_drbg,
		mbedtls_entropy_func, &g_entropy,
		NULL, MBEDTLS_CTR_DRBG_ENTROPY_LEN)) {
		      ...
		      const char *mode, const void *key, size_t key_length)
{
	...
	for (i = 0; i < sizeof(kCipher) / sizeof(*kCipher); ++i) {
	...
	for (i = 0; i < sizeof(kMode) / sizeof(*kMode); ++i) {
                            ...
                            const char *tag, size_t tag_length)
{
	...
	info = mbedtls_cipher_info_from_type(MBEDTLS_CIPHER_AES_256_CCM);
	if (!info) {
		return -ENOENT;
	}
	...
	if (mbedtls_cipher_setup(&ctx, info)                              ||
	    ...
	    mbedtls_cipher_set_iv(&ctx, ivptr, iv_length)                 ||
	    mbedtls_cipher_reset(&ctx)                                    ||
	    mbedtls_cipher_update(&ctx, input, length, output, &outlen)   ||
	    mbedtls_cipher_finish(&ctx, output + outlen, &outlen)         ||
	    mbedtls_cipher_check_tag(&ctx, tagptr, tag_length)            ){
