...
function generate()
{
	...
	json_str=$(jq -c '.areas."7".offset = .areas."6".offset |
               .areas."7".length = .areas."6".length' tmp/json0)
...
function check()
{
	...
	jq -c 'if (.areas."6".offset != .areas."7".offset) or (.areas."6".length != .areas."7".length)
