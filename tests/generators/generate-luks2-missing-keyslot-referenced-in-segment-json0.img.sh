...
# *** Description ***
#
# generate primary header with first segment having keyslots array with value
...
function generate()
{
	...
	# add missing keyslot reference in keyslots array of first segment
	jq -c 'def arr: ["segments", (getpath(["segments"]) | keys | min), "keyslots"];
               def missks: getpath(["keyslots"]) | keys | max | tonumber + 1 | tostring;
	       setpath(arr; getpath(arr) + [ missks ])' tmp/json0 > tmp/json_mod0
	json_str="$(< tmp/json_mod0)"
	test ${#json_str} -lt $((LUKS2_JSON_SIZE*512)) || exit 2
	test ${#json_str_orig} -lt ${#json_str} || exit 2

	_dd if=tmp/json_mod0 of=tmp/json0 bs=1 conv=notrunc
